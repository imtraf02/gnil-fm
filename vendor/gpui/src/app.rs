...
impl Application {
    ...
    /// Controls whether closing the last window ends the platform event loop.
    ///
    /// Session services can disable this before calling [`Application::run`] so they remain able
    /// to create a window in response to a later IPC request.
    pub fn quit_when_last_window_closed(self, quit: bool) -> Self {
        self.0
            .borrow()
            .platform
            .set_quit_when_last_window_closed(quit);
        self
    }
