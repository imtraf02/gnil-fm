use crate::{
    ...
    ExternalFileDragError, ExternalPaths, FileDropEvent, FontId, Global, GlobalElementId, GlyphId,
...
use perf_trace::WindowPerfTrace;
pub use prompts::*;
...
impl Window {
    pub(crate) fn new(
            ...
            move |request_frame_options| {
                ...
                if invalidator.is_dirty() || request_frame_options.force_render {
                    measure("frame duration", || {
                        handle
                            .update(&mut cx, |_, window, cx| {
                                let frame_started = window.perf_trace.start();
                                let draw_started = window.perf_trace.start();
                                let arena_clear_needed = window.draw(cx);
                                let draw_elapsed = draw_started
                                    .map_or(Duration::ZERO, |started| started.elapsed());
                                let submit_started = window.perf_trace.start();
                                window.present();
                                let submit_elapsed = submit_started
                                    .map_or(Duration::ZERO, |started| started.elapsed());
                                window.perf_trace.record_frame(
                                    frame_started,
                                    draw_elapsed,
                                    submit_elapsed,
                                );
        ...
        Ok(Window {
            ...
            rendered_frame: Frame::new(DispatchTree::new(cx.keymap.clone())),
            next_frame: Frame::new(DispatchTree::new(cx.keymap.clone())),
            ...
            perf_trace: WindowPerfTrace::from_env(),
            refreshing: false,
...
impl Window {
    ...
    pub fn start_external_file_drag(
        ...
        self.platform_window.start_external_file_drag(paths)
    }
    ...
    pub fn dispatch_event(&mut self, event: PlatformInput, cx: &mut App) -> DispatchEventResult {
        let dispatch_started = self.perf_trace.begin_input();
        ...
        self.perf_trace
            .record_dispatch(dispatch_started, self.invalidator.is_dirty());
        result
    }
