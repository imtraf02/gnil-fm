...
use crate::{
    AnyWindowHandle, Bounds, Decorations, ExternalFileDragError, ExternalPaths, Globals, GpuSpecs,
...
pub struct WaylandWindowState {
    ...
    imported_parent: Option<zxdg_imported_v2::ZxdgImportedV2>,
...
impl Drop for WaylandWindow {
    fn drop(&mut self) {
        ...
        if let Some(imported_parent) = &state.imported_parent {
            imported_parent.destroy();
        }
...
impl WaylandWindow {
    ...
    pub fn new(
        ...
        if params.kind == WindowKind::Floating && params.wayland_parent.is_none() {
            toplevel.set_parent(parent.as_ref());
        }

        let imported_parent = match (&params.wayland_parent, &globals.foreign_importer) {
            (Some(handle), Some(importer)) => {
                let imported =
                    importer.import_toplevel(handle.as_str().to_owned(), &globals.qh, ());
                imported.set_parent_of(&surface);
                Some(imported)
            }
            (Some(_), None) => {
                log::warn!(
                    "xdg-foreign v2 is unavailable; opening the external child as an independent toplevel"
                );
                None
            }
            (None, _) => None,
...
impl PlatformWindow for WaylandWindow {
    ...
    fn start_external_file_drag(
        ...
        state
            .client
            .start_external_file_drag(state.surface.clone(), paths)
    }
