...
use crate::{
    Action, Any, AnyView, AnyWindowHandle, AppContext, Asset, AssetSource, BackgroundExecutor,
    Bounds, ClipboardItem, CursorStyle, DispatchPhase, DisplayId, EventEmitter, FocusHandle,
...
/// Controls when the platform event loop exits.
#[derive(Clone, Copy, Debug, Default, Eq, PartialEq)]
pub enum QuitPolicy {
    /// Exit after the last window closes.
    #[default]
    LastWindowClosed,
    /// Keep the event loop alive until the application explicitly quits.
    Explicit,
}
...
impl Application {
    ...
    pub fn new() -> Self {
        ...
        Self(App::new_app(current_platform(), Arc::new(())))
    }
    ...
    /// Selects when the platform event loop exits.
    pub fn with_quit_policy(self, policy: QuitPolicy) -> Self {
        self.0.borrow().platform.set_quit_policy(policy);
        self
    }
