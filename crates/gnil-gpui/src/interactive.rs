...
impl ExternalPaths {
    ...
    pub fn to_uri_list(&self) -> std::result::Result<Vec<u8>, ExternalFileDragError> {
        if self.0.is_empty() {
            return Err(ExternalFileDragError::NoPaths);
        }
        let mut uri_list = String::new();
        for path in &self.0 {
            let path =
                std::path::absolute(path).map_err(|source| ExternalFileDragError::InvalidPath {
                    path: path.clone(),
                    source,
                })?;
            let uri = Url::from_file_path(&path)
                .map_err(|()| ExternalFileDragError::InvalidFileUri(path.clone()))?;
...
/// Why a native outbound file drag could not start.
#[derive(Debug, thiserror::Error)]
pub enum ExternalFileDragError {
    /// No paths were supplied.
    #[error("an external file drag requires at least one path")]
    NoPaths,
    /// A path could not be made absolute.
    #[error("cannot resolve external drag path {path:?}")]
    InvalidPath {
        /// The invalid path.
        path: PathBuf,
    ...
    /// An absolute path could not be encoded as a file URI.
    #[error("cannot encode external drag path {0:?} as a file URI")]
    InvalidFileUri(PathBuf),
    /// The compositor did not expose the required data-device protocol.
    #[error("external file drag is unavailable in this Wayland session")]
    Unavailable,
}

#[cfg(any(test, feature = "test-support"))]
#[derive(Debug, Clone, Copy, Default)]
pub(crate) struct FileDragEndedEvent;
...
impl PlatformInput {
    pub(crate) fn mouse_event(&self) -> Option<&dyn Any> {
        match self {
            ...
            PlatformInput::FileDragEnded => None,
        }
    }
