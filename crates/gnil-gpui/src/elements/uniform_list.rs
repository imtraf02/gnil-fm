...
use crate::{
    ...
    Overflow, Pixels, Point, ScrollHandle, Size, StyleRefinement, Styled, Window, point, size,
};
use smallvec::SmallVec;
use std::{cell::RefCell, cmp, ops::Range, rc::Rc};

/// The sizing behavior to apply during uniform-list layout.
#[derive(Clone, Copy, Debug, Default, PartialEq, Eq, PartialOrd, Ord, Hash)]
pub enum ListSizingBehavior {
    /// Calculate the list size from its items.
    Infer,
    /// Let the surrounding layout determine the list size.
    #[default]
    Auto,
}

/// Controls whether uniform-list items may exceed the list width.
#[derive(Clone, Copy, Debug, Default, PartialEq, Eq, PartialOrd, Ord, Hash)]
pub enum ListHorizontalSizingBehavior {
    /// Keep item widths within the list width.
    #[default]
    FitList,
    /// Allow items to make the list wider.
    Unconstrained,
}
...
impl Element for UniformList {
    ...
    fn request_layout(
        ...
        let item_size = if self.sizing_behavior == ListSizingBehavior::Infer {
            match self.item_height {
                Some(height)
                    if self.horizontal_sizing_behavior == ListHorizontalSizingBehavior::FitList =>
                {
                    size(Pixels::ZERO, height)
                }
                Some(height) => {
                    let mut item_size = self.measure_item(None, window, cx);
                    item_size.height = height;
                    item_size
                }
    ...
    fn prepaint(
        ...
        let longest_item_size = match self.item_height {
            Some(height) if !can_scroll_horizontally => size(padded_bounds.size.width, height),
            Some(height) => {
                let mut item_size = self.measure_item(None, window, cx);
                item_size.height = height;
                item_size
...
impl UniformList {
    /// Uses a known item height instead of rendering and laying out an extra item to measure it.
    ///
    /// This is useful for hot, virtualized lists whose row height is already part of their
    /// component contract. Horizontally unconstrained lists may still measure an item for width.
    pub fn item_height(mut self, height: Pixels) -> Self {
        assert!(
            height > Pixels::ZERO,
            "uniform-list item height must be positive"
        );
