...
use crate::{
    AnyWindowHandle, App, AsyncWindowContext, BackgroundExecutor, Bounds, DEFAULT_WINDOW_SIZE,
    DevicePixels, DispatchEventResult, ExternalFileDragError, ExternalPaths, Font, FontId,
    FontMetrics, FontRun, ForegroundExecutor, GlyphId, GpuSpecs, ImageSource, LineLayout, Pixels,
    PlatformInput, Point, QuitPolicy, RenderGlyphParams, RenderImage, RenderImageParams,
    RenderSvgParams, Scene, ShapedGlyph, ShapedRun, SharedString, Size, SvgRenderer, SvgSize,
    SystemWindowTab, TaskLabel, Window, WindowControlArea, hash, point, px, size,
};
use anyhow::Result;
...
pub(crate) trait Platform: 'static {
    ...
    fn quit(&self);
    fn set_quit_policy(&self, _policy: QuitPolicy) {}
...
pub(crate) trait PlatformWindow: HasWindowHandle + HasDisplayHandle {
    ...
    fn start_external_file_drag(
        &self,
        _paths: ExternalPaths,
    ) -> std::result::Result<(), ExternalFileDragError> {
        Err(ExternalFileDragError::Unavailable)
    }
...
pub struct WindowOptions {
    ...
    /// A parent owned by another process, imported with xdg-foreign v2.
    pub wayland_parent: Option<WaylandParentHandle>,
...
/// An xdg-foreign v2 handle for a parent window owned by another process.
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct WaylandParentHandle(String);

impl WaylandParentHandle {
    /// Creates a parent handle from an xdg-foreign v2 token.
