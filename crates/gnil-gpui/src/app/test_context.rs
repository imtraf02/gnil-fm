...
use crate::{
    ...
    Element, Empty, EventEmitter, ExternalPaths, FileDragEndedEvent, ForegroundExecutor, Global,
    ...
    TestDispatcher, TestPlatform, TestWindow, TextSystem, VisualContext, Window, WindowBounds,
...
impl VisualTestContext {
    ...
    pub fn take_external_file_drag(&mut self) -> Option<ExternalPaths> {
        self.cx
    ...
    /// Completes the native file drag most recently requested by the window.
    pub fn finish_external_file_drag(&mut self) {
        self.simulate_event(FileDragEndedEvent);
    }
